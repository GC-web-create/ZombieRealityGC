...
# ================== SOUND ==================
pygame.mixer.init()
if os.path.exists("shoot.ogg"):
    shoot_sound = pygame.mixer.Sound("shoot.ogg")
if os.path.exists("bg.ogg"):
...
def splash():
    screen.fill(BLACK)
    text = BIGFONT.render("GC", True, GREEN)
    title = FONT.render("Zombie Reality - GC", True, WHITE)
    dev = FONT.render("Developed by GC", True, WHITE)
    screen.blit(text, (430, 180))
    screen.blit(title, (320, 260))
    screen.blit(dev, (350, 300))
...
def menu():
    ...
    while running:
        ...
        screen.blit(FONT.render("Press S for Free Spin",1,WHITE),(300,200))
...
def game():
    ...
    while running:
        ...
        if elapsed >= GAME_TIME_LIMIT or lives <= 0:
            running = False
